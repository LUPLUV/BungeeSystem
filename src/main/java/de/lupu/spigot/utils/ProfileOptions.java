...
public class ProfileOptions {
    ...
    public static ProfileOptions loadProfile(UUID uuid){
        if(SpigotMain.getPlugin().getMySQL().isConnected()){
            if(SpigotMain.getPlugin().getFriendManager().isUserExists(uuid)){
                try {
    ...
    public void saveProfileOptions(){
        if(SpigotMain.getPlugin().getMySQL().isConnected()){
            if(SpigotMain.getPlugin().getFriendManager().isUserExists(uuid)){
                try{
                    PreparedStatement ps1 = SpigotMain.getPlugin().getMySQL().getCon()
                            .prepareStatement("UPDATE psys_FriendsTable " +
                                    "SET TOGGLE_REQUEST = ? WHERE UUID = ?");
                    PreparedStatement ps2 = SpigotMain.getPlugin().getMySQL().getCon()
                            .prepareStatement("UPDATE psys_FriendsTable " +
                                    "SET TOGGLE_ONLINE_NOTIFY = ? WHERE UUID = ?");
                    PreparedStatement ps3 = SpigotMain.getPlugin().getMySQL().getCon()
                            .prepareStatement("UPDATE psys_FriendsTable " +
                                    "SET CURRENT_SERVER = ? WHERE UUID = ?");
                    PreparedStatement ps4 = SpigotMain.getPlugin().getMySQL().getCon()
                            .prepareStatement("UPDATE psys_FriendsTable " +
                                    "SET LAST_ONLINE = ? WHERE UUID = ?");
                    PreparedStatement ps5 = SpigotMain.getPlugin().getMySQL().getCon()
                            .prepareStatement("UPDATE psys_FriendsTable " +
                                    "SET STATUS = ? WHERE UUID = ?");
                    PreparedStatement ps6 = SpigotMain.getPlugin().getMySQL().getCon()
                            .prepareStatement("UPDATE psys_FriendsTable " +
